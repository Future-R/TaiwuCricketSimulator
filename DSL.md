...
- **养剑**：`造成伤害后，若本次攻击为非暴击，若概率触发(33%)，层数增加10；计算属性时，若属性为暴击伤害，结果=基础值+层数`
- **红莲**：`防御时，若概率触发(50%)，反弹100%伤害`
- **玄甲**：`回合开始时，造成200%回合数的体力损伤`
...
对于逻辑过于复杂无法用 DSL 描述的技能（如“舍身”的复杂倍率、“变化”的属性转化），目前模拟器采用混合模式：保留 DSL 字符串用于显示和文档导出，但实际逻辑在代码中硬编码实现。